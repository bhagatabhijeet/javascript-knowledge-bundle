...
Reach for `map`/`filter`/`reduce` when the goal is to produce a new array or value from the original; reach for `for...of`/`forEach` when the goal is a side effect, like logging or updating something outside the array. See [Mapping an Array](./mapping-an-array.md), [Filtering an Array](./filtering-an-array.md), and [Reducing an Array](./reducing-an-array.md) for a closer look at each.
